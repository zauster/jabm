...
/**
 * <p>
 * The main application class for JABM experiments when running in headless
 * mode, e.g. on a cluster (to run using a GUI see {@link
 ...
 * net.sourceforge.jabm.SimulationManager</code>
 *  </p>
 *
 * <p>
 * System parameters:
 ...
 * <table border="1">
 * <tr>
 * <td><code>jabm.config</code></td> <td>The name of the Spring configuration
 * file defining the simulation model</td>
 * </tr>
 * <tr>
 * <td><code>jabm.propertyfile</code></td> <td>The name of a properties file
 * which overrides the values specified in the beans configuration</td>
 * </tr>
 * <tr>
 * <td><code>jabm.varfile</code></td> <td>The name of a properties file which
 * specifies the ranges of independent variables to be used for a multiple
 * treatment factor experiment</td>
 * </tr>
 * <td><code>jabm.configonly</code></td>  <td>If set to true in combintation
 * with jabm.varfile then create experiment property files without running any
 * simulations.  This is useful for running experiments on clusters.
 * </table>
 *
...
public class SimulationManager implements Runnable {
  ...
  public SimulationManager() {
    SystemProperties systemProperties = SystemProperties.jabsConfiguration();
    propFile = systemProperties.getProperty(SystemProperties.PROPERTY_PROPFILE);
    varFile = systemProperties.getProperty(SystemProperties.PROPERTY_VARFILE);
    ...
    generateSeeds = Boolean.parseBoolean(
        systemProperties.getProperty(SystemProperties.PROPERTY_SEEDS, "false"));
  ...
  public void run() {
    if (propFile != null) {
      ...
      if (varFile != null) {
        setup(varFile, baseDirName, generateSeeds, seedMask);
      } else {
        if (configOnly) {
          setup(baseDirName, generateSeeds, seedMask);
        } else {
          runSingleExperiment();
        }
      }
  ...
  public SimulationController getSimulationController() {
    return (SimulationController)BeanFactorySingleton.getBean(
        SIMULATION_CONTROLLER_BEAN);
  }

  public void runSingleExperiment() { launch(getSimulationController()); }
  ...
  public void runSingleExperiment(String propFile) {
    try {
      logger.info("Running single experiment with properties " + propFile);
  ...
  public void setup(String varFile, String baseDirName, boolean generateSeeds,
                    int seedMask) {
    ...
    VariableBindingsIterator iterator =
        new VariableBindingsIterator(varFile, beanFactory);

    while (iterator.hasNext()) {
      Map<String, String> bindings = iterator.next();
      SimulationExperiment experiment =
          new SimulationExperiment(beanFactory, baseDirName, experimentNumber++,
                                   bindings, generateSeeds, seedMask);
  ...
  public void setup(String baseDirName, boolean generateSeeds, int seedMask) {
    ...
    SimulationExperiment experiment = new SimulationExperiment(beanFactory, baseDirName, 0, emptyBindings,
